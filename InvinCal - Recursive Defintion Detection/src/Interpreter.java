...
public class Interpreter 
{
	...
	{	
		//System.out.println(standardizedString);
		...
		if(operator == null)
		{
			...
			//System.out.println(content.toString() + ": " + isValue(content));
			
			if(Constant.isConstant(content))
			{				
				return Constant.getConstant(content);
			
			}
			else if(isValue(content) && !content.isEmpty())
			{
				//System.out.println("ISVALUE");
		...
		//System.out.println("SUBELEMENTS: " + Arrays.toString(subElementStandardizedStrings));
...
//			System.out.println("getContent - string: " + string);
	 ...
	 * @return
	 */
	public static Operator getOperator(InterpreterContentAnalyzer contentAnalyze)
	{
	...
	/**
	 * Check if the content is a value.
	 * Being a value, the content has only numbers and '.'
	 * If there's '.', there should be something behind '.' (.9 is acceptable)
	 * @param content
	 * @return
	 */
	public static boolean isValue(String content)
	{
		...
		List<String> valueCharacters = Arrays.asList(v);
		
		// if content contains characters not included in valueCharacters
		for(int i = 0; i < content.length(); i++)
		{
			String c = content.substring(i, i+1);
			
			if(!valueCharacters.contains(c))
			{
				return false;
			}
		}
		
		// if content have more than 1 '.', the following boolean expression would be true
		if(content.indexOf(".") != content.lastIndexOf("."))
		{
			return false;
		}
		
		// for cases like '9.', false is returned 
		if(content.endsWith("."))
		{
			return false;
		}
 ...
 *
 */
class InterpreterContentAnalyzer 
{
	...
	public Operator getFunctionOperator()
	{
		...
		if(Function.isFunction(name))
		{
			...
			return Function.getFunctionList().get(name);
		}
		else
		{
			return null;
	...
	/**
	 * This is the specific method for getting sub-elements from a expression with function operator
	 * @return
	 */
