...
/**
 * Standardizer standardizes a source formula string into standardized form which can be interpreted by the interpreter
...
public class Standardizer 
{
	...
	{
		String s = standardize("-(-5)/-(3.6-4*factorial(2))/-(4.2)+4");		// -(-7)/-5/+43(654.33 - 4.2)
...
//			System.out.println("BOIXNG: BOXED- " + boxed);
	...
	/**
	 * This function adds * symbol to locations where are multiplication but the * symbol is implicit.
...
//		System.out.println("FORMERSTRINGISNOTAFUNCTIONNAME: FUNCTIONNAME- " + functionName);
		...
		return !Operator.operatorList.containsKey(functionName);
		
	}
	
	/**
	 * Standardize the formula string so that the brackets are correct (the enclosing brackets are not added in this method). All of the sub-elements are standardized along the way.
...
//		System.out.println("BRACKETSTANDARDIZE!");
	...
	private static class StandardizerStringAnalyzer
	{
		...
		{
			for(String functionName : Operator.operatorList.keySet())
			{
				String charAt0 = "";
